  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-white text-slate-900">
      ...
      <div className="mb-6 border rounded-lg p-4 bg-amber-50">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <h2 className="text-lg font-semibold text-slate-900 mb-2">Planning Officiel Formation HORL1S925</h2>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="inline-flex rounded-md shadow-sm border border-slate-200 overflow-hidden bg-white">
            <button aria-label="Semaine précédente" className="px-3 py-2 hover:bg-slate-50" onClick={prevWeek}>
              <ChevronLeft className="w-4 h-4" />
            </button>
            <button className="px-3 py-2 border-l border-r border-slate-200 hover:bg-slate-50" onClick={thisWeek}>Cette semaine</button>
            <button aria-label="Semaine suivante" className="px-3 py-2 hover:bg-slate-50" onClick={nextWeek}>
      ...
      <div className="mt-4 grid grid-cols-7 gap-2 text-center text-sm">
        {weekDays.map((d, i) => (
          <button key={i} onClick={() => { setSelectedDay(i); if (view === 'week') setView('day'); }} className={`rounded-md border py-2 ${selectedDay === i ? 'bg-amber-50 border-amber-300 text-amber-900' : 'bg-white hover:bg-slate-50 text-slate-700'}`}>
      ...
      {view === 'week' ? (
        <div className="mt-4 overflow-x-auto">
          <div className="min-w-[720px] grid grid-cols-8 border rounded-md bg-white">
            ...
            {DAYS.map((d) => (<div className="bg-slate-50 p-2 text-xs text-slate-600 text-center" key={d}>{d}</div>))}
            {HOURS.map((h) => (
              <>
                <div className="border-t p-2 text-xs text-slate-500 bg-white" key={`h-${h}`}>{String(h).padStart(2, '0')}:00</div>
                {DAYS.map((_, dayIdx) => (
                  <div className="border-t border-l relative h-16 bg-white hover:bg-amber-50/60 cursor-pointer" key={`c-${h}-${dayIdx}`} onDoubleClick={() => openCreate(dayIdx)} title="Double-clic pour ajouter">
                    {entries.filter((e) => e.day === dayIdx && e.start <= `${String(h).padStart(2, '0')}:59` && e.end >= `${String(h).padStart(2, '0')}:00`).map((e) => (
                      <div key={e.id} onClick={(ev) => { ev.stopPropagation(); openEdit(e); }} className="absolute left-1 right-1 top-1 rounded-md text-xs text-slate-900 shadow border border-slate-200" style={{ backgroundColor: e.color || '#f1f5f9' }}>
        ...
        <div className="mt-4 grid gap-3 lg:grid-cols-3">
                  ...
                  {dayEntries.filter((e) => e.start <= `${String(h).padStart(2, '0')}:59` && e.end >= `${String(h).padStart(2, '0')}:00`).map((e) => (
                    <div key={e.id} className="mb-2 rounded-md text-xs text-slate-900 shadow border border-slate-200 bg-white flex items-center justify-between" style={{ backgroundColor: e.color || '#f8fafc' }}>
                      ...
                      <div className="flex items-center gap-1 pr-1">
                        ...
                        <button className="p-1 hover:bg-slate-50 rounded" onClick={() => remove(e.id)} aria-label="Supprimer"><Trash2 className="w-4 h-4 text-slate-700" /></button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          <div className="border rounded-md p-3 h-fit bg-white">
            ...
            <ul className="space-y-2 text-sm">
              {dayEntries.length === 0 && <li className="text-slate-500">Aucune tâche pour ce jour.</li>}
              {dayEntries.map((e) => (
                <li className="border rounded p-2 flex items-center justify-between bg-white" key={e.id}>
                  <div className="min-w-0">
                    <div className="font-medium truncate">{e.title}</div>
                    <div className="text-xs text-slate-500">{e.start}–{e.end}</div>
                  </div>
                  <div className="flex items-center gap-2">
                    <button className="p-1 hover:bg-slate-50 rounded" onClick={() => openEdit(e)}><Pencil className="w-4 h-4 text-slate-700" /></button>
                    <button className="p-1 hover:bg-slate-50 rounded" onClick={() => remove(e.id)}><Trash2 className="w-4 h-4 text-slate-700" /></button>
                  </div>
                </li>
              ))}
            </ul>
        ...
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/30 p-2 sm:p-6">
          <div className="w-full max-w-lg bg-white rounded-lg shadow-lg">
            <div className="border-b px-4 py-3 flex items-center justify-between">
              ...
              <button className="p-1 hover:bg-slate-100 rounded" onClick={() => setShowForm(false)} aria-label="Fermer">✕</button>
            </div>
