    ...
    <div className="min-h-screen bg-black text-green-400 p-4 font-mono">
      <div className="bg-slate-900 border border-slate-700 rounded-lg p-4 mb-4">
        <div className="flex items-center justify-between">
          ...
          <Link href="/outils/chronographe-banc-pro" className="bg-green-900 hover:bg-green-800 px-4 py-2 border border-green-700 flex items-center gap-2">← Retour au chronographe</Link>
        </div>
      </div>
      <div className="bg-slate-900 border border-slate-700 rounded-lg p-4">
        ...
        {measurements && measurements.length > 0 ? (
            ...
            {measurements.map((meas: any) => (
              <div key={meas.id} className="bg-slate-800 border border-slate-700 rounded p-4 hover:border-green-700 transition-colors">
                <div className="flex justify-between items-start mb-2">
                  ...
                  <span className="text-xs text-slate-500">{new Date(meas.created_at).toLocaleDateString('fr-CH')}</span>
                </div>
                <div className="grid grid-cols-2 gap-2 text-sm">
                  ...
                  <div>Rate: <span className={`${Math.abs(meas.rate_avg) > 10 ? 'text-red-400' : 'text-green-400'}`}>{meas.rate_avg > 0 ? '+' : ''}{meas.rate_avg?.toFixed(1)}s/j</span></div>
          ...
          <div className="text-center py-16 text-slate-600"><p>Aucune mesure publique pour l'instant.</p><p className="text-xs mt-2">Soyez le premier à publier une mesure !</p></div>
        )}
      </div>
    </div>
  );
