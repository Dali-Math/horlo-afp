...
/* =========================
   Composant de carte
========================= */
function MaterialCard({ icon, title, colorClass, description, useCases, illustration }: Material) {
  return (
    <article className="group relative bg-white/90 dark:bg-slate-900/40 rounded-3xl overflow-hidden border border-slate-200/50 dark:border-slate-700/50 hover:shadow-xl transition-all duration-500 hover:-translate-y-1 backdrop-blur-sm">
      ...
      <div className="p-6 flex flex-col space-y-3">
        <div className="flex items-start gap-3">
          ...
          <h3 className="text-xl font-bold text-slate-900 dark:text-white group-hover:text-amber-500 transition">{title}</h3>
        </div>
    ...
    <main className="min-h-screen bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 text-white">
        ...
        <div className="relative max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-16 md:py-20">
          <div className="flex items-center gap-2 text-sm text-gray-400 mb-8">
            <Link href="/suisse" className="hover:text-amber-400 transition-colors flex items-center gap-1">
              <ArrowLeft className="w-4 h-4" /> Horlogerie Suisse
            </Link>
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-start">
            <div>
              ...
              <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold mb-6 leading-tight">
                <span className="text-white">Excellence et</span><br />
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 via-amber-500 to-orange-500">Innovation</span><br />
                <span className="text-white">dans les Matériaux Horlogers</span>
              </h1>
              <p className="text-lg text-gray-300 mb-6">L’union parfaite entre tradition, recherche et design suisse. Découvrez les grandes régions horlogères et leurs savoir-faire.</p>
              <a href="/materiaux-horlogers-suisse.html#hero" className="inline-flex items-center gap-2 px-4 py-2 rounded-md bg-amber-500/10 border border-amber-500/30 text-amber-300 hover:bg-amber-500/20 transition">
      ...
      <section className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 pt-10 pb-6">
        <div className="rounded-2xl border border-amber-500/10 bg-slate-900/40 backdrop-blur-md shadow-xl">
          ...
          <div className="p-6 sm:p-8">
            <h2 className="text-2xl sm:text-3xl font-extrabold text-amber-400 mb-4">{selected.title}</h2>
            <p className="text-gray-300 leading-relaxed mb-8 max-w-4xl">{selected.description}</p>
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                <h3 className="flex items-center gap-2 font-semibold text-white mb-3"><Factory className="w-5 h-5 text-amber-400" /> Spécialités</h3>
                <ul className="space-y-2 text-gray-300">{selected.specialties.map((s, i) => (<li key={i} className="flex gap-2"><span className="text-amber-400">•</span><span>{s}</span></li>))}</ul>
              </div>
              <div>
                <h3 className="flex items-center gap-2 font-semibold text-white mb-3"><Landmark className="w-5 h-5 text-amber-400" /> Entreprises</h3>
                <div className="flex flex-wrap gap-2">{selected.companies.map((c, i) => (<span key={i} className="px-3 py-1.5 rounded-full text-sm bg-amber-500/10 text-amber-300 border border-amber-500/20">{c}</span>))}</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-10">
        <div className="flex flex-wrap gap-3 mb-8">
          {CATEGORIES.map(cat => (
            <button key={cat} onClick={() => setFilter(cat)} className={`px-5 py-2 rounded-2xl font-semibold text-sm transition-all ${filter === cat ? 'bg-gradient-to-r from-amber-500 to-amber-600 text-white shadow-md' : 'bg-slate-800 text-slate-300 hover:bg-slate-700'}`}>{cat}</button>
          ))}
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">{filtered.map((m, i) => (<MaterialCard key={i} {...m} />))}</div>
      </section>

      {/* CHRONOLOGIE HISTORIQUE */}
      <section className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 py-16">
        <h2 className="text-3xl font-bold text-center text-amber-400 mb-4">Chronologie Historique</h2>
        <p className="text-center text-gray-300 mb-10">L’évolution des matériaux horlogers du XVIe siècle à aujourd’hui</p>
        <Chronologie />
      </section>
