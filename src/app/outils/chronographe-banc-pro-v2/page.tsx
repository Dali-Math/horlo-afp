...
import type { TimingData } from '@/data/types';
import { 
...
export default function ChronographeBancPro() {
  ...
  const diagnose = useCallback((stats: any, calibreId: string) => {
    ...
    if (stats.amplitude.avg < calibre.minAmplitude) {
      issues.push(`Amplitude critique: ${stats.amplitude.avg.toFixed(0)}° < ${calibre.minAmplitude}°`);
    }
    if (stats.jitter.avg > 0.5) {
      issues.push("Signal irrégulier - vérifier escapement");
    }
    if (stats.beatError.abs > calibre.maxBeatError) {
      issues.push(`Beat error élevé: ${stats.beatError.abs.toFixed(1)}ms > ${calibre.maxBeatError}ms`);
    }
    if (stats.qFactor.avg < 1500) {
      issues.push(`Q factor faible: ${stats.qFactor.avg.toFixed(0)} - balancier usé?`);
    }
    if (profile?.typicalIssues) {
      issues.push(...profile.typicalIssues);
    }
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(24);
    doc.text('HORLOLEARN', 105, 30, { align: 'center' });
    
    doc.setFontSize(16);
    doc.text('Rapport Chronographe de Banc Pro', 105, 45, { align: 'center' });
    
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(12);
    doc.text(`Généré le: ${new Date().toLocaleDateString('fr-CH', {
      year: 'numeric',
      month: 'long',
      day: 'numeric',
      hour: '2-digit',
    ...
    doc.text(`Température: ${temperature}°C`, 20, 95);
    doc.text(`Durée: ${measurementDuration}s`, 20, 105);
    
    // Tableau des résultats
    let yPos = 120;
    doc.setFont('helvetica', 'bold');
    doc.text('RÉSULTATS PAR POSITION', 20, yPos);
    yPos += 10;
    
    doc.setFontSize(10);
    doc.text('Position', 20, yPos);
    doc.text('Amplitude', 70, yPos);
    doc.text('Rate', 110, yPos);
    doc.text('Beat Error', 150, yPos);
    doc.text('Q Factor', 190, yPos);
    yPos += 5;
    
    doc.setFont('helvetica', 'normal');
    Object.entries(averages).forEach(([pos, stats]: [string, any]) => {
      yPos += 8;
      doc.text(POSITIONS.find(p => p.id === pos)?.name || pos, 20, yPos);
      doc.text(`${stats.amplitude.avg.toFixed(1)}°`, 70, yPos);
      doc.text(`${stats.rate.avg > 0 ? '+' : ''}${stats.rate.avg.toFixed(1)}s/j`, 110, yPos);
      doc.text(`${stats.beatError.abs.toFixed(1)}ms`, 150, yPos);
      doc.text(`${stats.qFactor.avg.toFixed(0)}`, 190, yPos);
    });
    
    // Statut COSC
    yPos += 15;
    const isCOSC = Object.values(averages).every((stats: any) => stats.isCOSC);
    if (isCOSC) {
      doc.setTextColor(0, 255, 0);
      doc.setFont('helvetica', 'bold');
      doc.text('✓ COMPLIANT COSC', 105, yPos, { align: 'center' });
    } else {
      doc.setTextColor(255, 0, 0);
      doc.text('✗ NON-COMPLIANT COSC', 105, yPos, { align: 'center' });
    }
    ...
    doc.text('Generated with HorloLearn Pro v4.0', 105, 290, { align: 'center' });
  ...
  // EXPORT CSV
  const exportCSV = () => {
    const headers = ['Position', 'Amplitude Moy (°)', 'Ecart-type', 'Beat Error (ms)', 
                    'Rate (s/j)', 'Jitter (ms)', 'Q Factor', 'Isochronisme (%)'];
    ...
    a.href = url;
    a.download = `horlolearn_${Date.now()}.csv`;
    a.click();
  };

  // PUBLIER À LA GALLERY
  const publishToGallery = async () => {
    try {
      const inserts = Object.entries(averages).map(([pos, stats]: [string, any]) => ({
        ...
        amplitude_avg: stats.amplitude.avg,
        rate_avg: stats.rate.avg,
  ...
  return (
    <div className="min-h-screen bg-black text-green-400 p-4 font-mono select-none">
      ...
      {showSavedModal && (
        <div className="fixed top-4 right-4 bg-green-900 border border-green-700 px-4 py-2 rounded shadow-lg z-50">
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-lg p-4 mb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-xl font-bold">CHRONOGRAPHE PRO v4.1</h1>
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-lg p-4 mb-4">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          ...
          <div>
            <label className="block text-slate-400 text-xs mb-1 flex items-center gap-1">
              CALIBRE <Info className="w-3 h-3 cursor-help" onMouseEnter={showTooltip("Sélection auto-configure les paramètres")} onMouseLeave={hideTooltip} />
            </label>
          ...
          <div>
            <label className="block text-slate-400 text-xs mb-1 flex items-center gap-1">
              DURÉE (s) <Info className="w-3 h-3 cursor-help" onMouseEnter={showTooltip("Durée minimale 20-30s")} onMouseLeave={hideTooltip} />
            </label>
        ...
        <div className="mb-4 p-3 bg-slate-800/50 rounded border border-slate-700">
          <label className="block text-slate-400 text-xs mb-2 flex items-center gap-1 font-bold">
            <Activity className="w-3 h-3" />
            PROFIL DE SIGNATURE <Info className="w-3 h-3 cursor-help" onMouseEnter={showTooltip("Simule le comportement réel d'un mouvement")} onMouseLeave={hideTooltip} />
          </label>
          <select value={selectedProfile} onChange={(e) => setSelectedProfile(e.target.value)} disabled={isRunning} className="w-full bg-black border border-slate-700 p-2 text-green-400 focus:border-green-500 focus:outline-none mb-2">
            {SIGNATURE_PROFILES.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
          </select>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-slate-400 text-xs mb-1">BEAT RATE</label>
            <select value={selectedRate} onChange={(e) => setSelectedRate(Number(e.target.value))} disabled={isRunning} className="w-full bg-black border border-slate-700 p-2 text-green-400 focus:border-green-500 focus:outline-none">
              <option value="18000">18,000 A/h | 2.5 Hz</option>
              <option value="21600">21,600 A/h | 3 Hz</option>
              <option value="25200">25,200 A/h | 3.5 Hz</option>
              <option value="28800">28,800 A/h | 4 Hz</option>
              <option value="36000">36,000 A/h | 5 Hz</option>
              <option value="43200">43,200 A/h | 6 Hz</option>
              <option value="57600">57,600 A/h | 8 Hz</option>
              <option value="72000">72,000 A/h | 10 Hz</option>
            </select>
          </div>
        ...
        <div className="flex gap-2 mt-4 flex-wrap">
          <button onClick={handleStart} disabled={isRunning} className="bg-green-900 hover:bg-green-800 disabled:bg-slate-800 px-4 py-2 border border-green-700 flex items-center gap-2 transition-colors touch-friendly">
            <Play className="w-4 h-4" /> START [Space]
          </button>
          <button onClick={handleStop} disabled={!isRunning} className="bg-red-900 hover:bg-red-800 disabled:bg-slate-800 px-4 py-2 border border-red-700 flex items-center gap-2 transition-colors touch-friendly">
            <Square className="w-4 h-4" /> STOP [Space]
          </button>
          <button onClick={handleReset} className="bg-slate-700 hover:bg-slate-600 px-4 py-2 border border-slate-600 flex items-center gap-2 transition-colors touch-friendly">
            <RotateCw className="w-4 h-4" /> RESET [Ctrl+R]
          </button>
          
          <div className="border-l border-slate-700 mx-2"></div>
          
          <button onClick={() => setAudioFeedback(!audioFeedback)} className={`px-4 py-2 border flex items-center gap-2 transition-colors touch-friendly ${audioFeedback ? 'bg-blue-900 hover:bg-blue-800 border-blue-700' : 'bg-slate-700 hover:bg-slate-600 border-slate-600'}`}>
            <Volume2 className="w-4 h-4" /> AUDIO {audioFeedback ? 'ON' : 'OFF'}
          </button>
          <button onClick={() => setShowGraph(!showGraph)} className={`px-4 py-2 border flex items-center gap-2 transition-colors touch-friendly ${showGraph ? 'bg-purple-900 hover:bg-purple-800 border-purple-700' : 'bg-slate-700 hover:bg-slate-600 border-slate-600'}`}>
            <BarChart2 className="w-4 h-4" /> GRAPHIQUE
          </button>
          <button onClick={() => document.documentElement.requestFullscreen()} className="bg-slate-700 hover:bg-slate-600 px-4 py-2 border border-slate-600 flex items-center gap-2 transition-colors touch-friendly">
            <Maximize2 className="w-4 h-4" /> PLEIN ÉCRAN
          </button>
          
          <div className="border-l border-slate-700 mx-2"></div>
          
          <button onClick={exportPDF} disabled={Object.keys(averages).length === 0} className="bg-purple-900 hover:bg-purple-800 disabled:bg-slate-800 px-4 py-2 border border-purple-700 flex items-center gap-2 transition-colors touch-friendly">
            <FileText className="w-4 h-4" /> PDF
          </button>
          <button onClick={exportCSV} disabled={Object.keys(averages).length === 0} className="bg-blue-900 hover:bg-blue-800 disabled:bg-slate-800 px-4 py-2 border border-blue-700 flex items-center gap-2 transition-colors touch-friendly">
            <Download className="w-4 h-4" /> CSV
          </button>
          <button onClick={publishToGallery} disabled={Object.keys(averages).length === 0} className="bg-green-700 hover:bg-green-600 disabled:bg-slate-800 px-4 py-2 border border-green-600 flex items-center gap-2 transition-colors touch-friendly">
      ...
      {isRunning && latest && (
        <div className="bg-slate-900 border border-slate-700 rounded-lg p-4 mb-4">
          <div className="grid grid-cols-2 md:grid-cols-6 gap-4">
            ...
            <div className="bg-black p-3 rounded border border-slate-800">
              <div className="text-slate-500 text-xs">Q FACTOR</div>
            ...
            <div className="bg-black p-3 rounded border border-slate-800">
              ...
              <div className={`text-2xl font-bold flex items-center gap-1 ${latest.jitter < 0.3 ? 'text-green-400' : 'text-yellow-400'}`}>
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            {Object.values(averages).some((a: any) => a?.isCOSC) && 
              <div className="bg-green-900 text-green-400 px-3 py-1 rounded text-sm flex items-center gap-1 font-bold">
                <CheckCircle className="w-4 h-4" />COSC COMPLIANT
              </div>
            }
          </div>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-700">
                  ...
                  <th className="text-right py-2 text-slate-400">BEAT ERR</th>
                  <th className="text-right py-2 text-slate-400">RATE</th>
                  <th className="text-right py-2 text-slate-400">Q FACTOR</th>
                  <th className="text-right py-2 text-slate-400">JITTER</th>
              ...
              <tbody>
                {Object.entries(averages).map(([pos, stats]: [string, any]) => (
                  <tr key={pos} className="border-b border-slate-800 hover:bg-slate-800/30">
                    ...
                    <td className={`text-right py-2 font-semibold ${Math.abs(stats.rate.avg) > 10 ? 'text-red-400' : Math.abs(stats.rate.avg) > 5 ? 'text-yellow-400' : 'text-green-400'}`}>
                    ...
                    <td className="text-center py-2">
                      {stats.isCOSC ? <CheckCircle className="w-4 h-4 text-green-400 inline" /> : <AlertTriangle className="w-4 h-4 text-yellow-400 inline" />}
                    </td>
                  </tr>
                ))}
              </tbody>
