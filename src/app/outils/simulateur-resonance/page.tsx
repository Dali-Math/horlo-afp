...
import { MagnetismResult, ImpactResult, Diagnostic } from '@/lib/simulateur/types';
import { calculateMagnetism } from '@/lib/simulateur/modules/magnetism';
import { simulateDrop } from '@/lib/simulateur/modules/impact';
import { calculatePowerReserve } from '@/lib/simulateur/physics-engine';
...
export default function SimulateurResonance3D() {
  ...
  const exportReport = () => {
    ...
    const metrics = [
      `Isochronisme: ${magnetismResult ? magResult.beatRateDeviation.toFixed(2) + ' Hz' : '-'}`,
      `Magnétisme: ${magnetismResult ? magResult.level.toFixed(0) + '%' : '-'}`,
