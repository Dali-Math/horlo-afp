...
// Fonts Next.js (optionnel mais premium)
  ...
  useEffect(() => {
    ...
    const loadScripts = async () => {
      return new Promise<void>((resolve) => {
        ...
        script1.onerror = () => { console.error('Erreur: Impossible de charger THREE.js'); resolve() }
    ...
    const initVanta = () => {
      ...
      } catch (error) { console.error('Erreur initialisation VANTA:', error) }
    }
    ...
    loadScripts().then(() => {
      setTimeout(() => {
        if (window.VANTA && threeReadyRef.current) initVanta()
      }, 300)
    })
  ...
  return (
    <div className={`font-sans bg-cream overflow-x-hidden ${inter.className}`} style={{ background: 'var(--cream)' }}>
      <style jsx global>{`
        ...
        body { font-family: 'Inter', sans-serif; background: var(--cream); overflow-x: hidden; }
        ...
        .scroll-reveal { opacity: 0; transform: translateY(30px); transition: all 0.6s ease; } .scroll-reveal.revealed { opacity: 1; transform: translateY(0); }
        .innovation-card { backdrop-filter: blur(10px); background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); transition: all 0.4s ease; position: relative; overflow: hidden; }
