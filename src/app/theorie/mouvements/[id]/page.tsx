...
// ✅ Calcul unique au chargement du module (hors du composant)
...
const SectionCard = ({ children, className = '' }: { children: React.ReactNode; className?: string }) => (
  <div className={`bg-white dark:bg-slate-900 rounded-xl p-8 border border-slate-200 dark:border-slate-700 ${className}`}>
    {children}
  </div>
);
...
const MetaRow = React.memo(({ label, value }: { label: string; value: number }) => (
...
const DescriptionTab = React.memo(({ concept }: { concept: Concept }) => (
  <div className="space-y-6">
    ...
    {concept.details?.materials && concept.details.materials.length > 0 && (
      <SectionCard>
        <SectionTitle>Matériaux & Composants</SectionTitle>
    ...
    {concept.details?.commonErrors && concept.details.commonErrors.length > 0 && (
      <SectionCard>
        <SectionTitle icon={<AlertTriangle className="w-6 h-6 text-amber-500" />}>
          Erreurs Courantes
        </SectionTitle>
    ...
    {/* Quiz intégré */}
    <SectionCard>
      <SectionTitle>Quiz Rapide</SectionTitle>
...
const SpecsTab = React.memo(({ concept }: { concept: Concept }) => (
  <div className="space-y-6">
    {/* Dimensions */}
    {concept.details?.dimensions && (
      <SectionCard>
        <SectionTitle>Dimensions & Tolérances</SectionTitle>
    ...
    {concept.details?.tools && concept.details.tools.length > 0 && (
      <SectionCard>
        <SectionTitle>Outils Requis</SectionTitle>
    ...
    {concept.details?.adjustment && (
      <SectionCard>
        <SectionTitle>Procédure de Réglage</SectionTitle>
        <p className="text-slate-700 dark:text-slate-300 leading-relaxed">
          {concept.details.adjustment}
        </p>
      </SectionCard>
    ...
    {/* Calculatrice pour dilatation thermique */}
...
const ApplicationsTab = React.memo(({ concept }: { concept: Concept }) => (
  <div className="space-y-6">
    {/* Galerie d'images */}
    {concept.gallery && concept.gallery.length > 0 && (
      <SectionCard>
        <SectionTitle>Galerie Technique</SectionTitle>
    ...
    {concept.manufactures && concept.manufactures.length > 0 && (
      <SectionCard>
        <SectionTitle>Applications dans les Montres</SectionTitle>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
    ...
    {concept.history && (
      <SectionCard>
        <SectionTitle icon={<Clock className="w-6 h-6" />}>
          Historique
        </SectionTitle>
        <p className="text-slate-700 dark:text-slate-300 leading-relaxed">
          {concept.history}
...
const ResourcesTab = React.memo(({ concept }: { concept: Concept }) => (
  <div className="space-y-6">
    {/* Normes ISO */}
    {concept.iso && concept.iso.length > 0 && (
      <SectionCard>
        <SectionTitle>Normes ISO</SectionTitle>
        <div className="space-y-3">
    ...
    {concept.patent && concept.patent.length > 0 && (
      <SectionCard>
        <SectionTitle>Brevets Historiques</SectionTitle>
      ...
      <SectionCard>
        ...
        <div className="space-y-2">
          {concept.relatedConcepts.map((relatedId) => {
            const related = ALL_CONCEPTS.find(c => c.id === relatedId);
            return related ? (
              <Link key={relatedId} href={`/theorie/mouvements/${relatedId}`} prefetch>
    ...
    <main className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
          ...
          <aside className="space-y-6">
            ...
            <SectionCard>
              ...
              <div className="space-y-2">
                ...
                <button className="text-sm text-blue-600 hover:text-blue-800 dark:hover:text-blue-600 transition-colors">
            ...
            <div className="bg-gradient-to-br from-purple-500 to-pink-600 rounded-xl p-6 text-white">
              ...
              <button className="w-full px-4 py-2 bg-white text-purple-600 rounded-lg font-bold hover:bg-slate-100 transition-colors">
                Démarrer le Quiz
              </button>
            </div>
            ...
            <SectionCard>
              ...
              <div className="space-y-2 text-sm">
                <MetaRow label="Tags" value={concept.tags?.length || 0} />
                <MetaRow label="Manufactures" value={concept.manufactures?.length || 0} />
                <MetaRow label="Normes ISO" value={concept.iso?.length || 0} />
              </div>
            </SectionCard>
          </aside>
        </div>
      ...
      <section className="max-w-7xl mx-auto px-4 pb-16">
        <nav className="flex justify-between items-center py-8 border-t border-slate-200 dark:border-slate-700" aria-label="Navigation entre concepts">
          {prev ? (
            <Link href={`/theorie/mouvements/${prev.id}`} prefetch className="flex items-center gap-3 p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-blue-500 transition-all max-w-xs group">
          ...
          {next ? (
            <Link href={`/theorie/mouvements/${next.id}`} prefetch className="flex items-center gap-3 p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 hover:border-blue-500 transition-all max-w-xs text-right group">
              <div>
