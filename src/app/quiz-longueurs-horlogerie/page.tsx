...
export default function QuizLongueursHorlogeriePage() {
  ...
  const TIME_PER_QUESTION = 20; // ⏱️ augmenté à 20 secondes
  ...
  return (
    <main className="min-h-screen bg-[#0a0a0a] flex flex-col items-center justify-center px-4 py-10">
      <motion.section
        ...
        className="w-full max-w-6xl bg-white rounded-3xl shadow-2xl p-12"
      >
        {!done ? (
          <>
          ...
          <div className="text-center">
            <h2 className="text-4xl font-bold text-[#E2B44F] mb-6">
              Résultats du Quiz
            </h2>
            ...
            {answers.some(a => !a.isCorrect) && (
              <div className="mt-8 text-left">
                <h3 className="text-2xl font-semibold mb-4 text-[#E2B44F]">Révisions</h3>
                <ul className="space-y-3">
                  {answers.filter(a => !a.isCorrect).map((a, idx) => (
                    <li key={idx} className="rounded-xl border p-4 bg-[#fafafa]">
            ...
            <button
              onClick={restart}
              className="mt-10 px-8 py-4 rounded-lg font-semibold bg-[#E2B44F] text-black hover:bg-[#f5cc66] transition text-lg"
            >
